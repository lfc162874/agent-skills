...
description: 根据招标文件、评分标准、技术卷大纲和知识库证据生成、改写、审查投标技术卷正文。用于技术卷、技术标、技术方案、实施方案、需求分析、项目理解、系统现状分析、质量保障、进度保障、售后服务、评分点响应、正文 JSON blocks 生成等场景；强制输出结构化 JSON，避免 Markdown、手写编号、乱造标题和无证据事实。
version: 1.0.0
language: zh-CN
...
7. 生成符合 `schemas/output-schema.json` 的 JSON blocks；
8. 读取 `docs/forbidden-content.md`，清除 Markdown、手写编号和不合规内容；
9. 根据 `validators/quality-checklist.md` 完成自检；
10. 只输出最终 JSON，不输出解释、分析过程或 Markdown 代码块。
